#!/bin/bash
# Deploy using existing Docker image (after docker login)
...
echo "=== Pulling Docker image (you must be logged in) ==="
cd /opt/academyfirstaid
sudo docker pull hackohackob/academyfirstaid:latest || {
    echo "⚠️  Image pull failed. The image may not exist on Docker Hub yet."
    echo "Options:"
    echo "1. Push your code to GitHub to trigger GitHub Actions build"
    echo "2. Or build the image locally on the server"
    echo ""
    echo "To build locally, run:"
    echo "  git clone https://github.com/hackohackob/academyfirstaid-learning-app.git /opt/academyfirstaid/source"
    echo "  cd /opt/academyfirstaid/source"
    echo "  sudo docker build -t hackohackob/academyfirstaid:latest ."
    exit 1
}
