#!/bin/bash
# Fixed version that handles Docker permissions
...
# Try with sudo first (works immediately)
if sudo docker-compose up -d 2>/dev/null; then
    echo "✅ Container started with sudo"
else
    # If that fails, try activating docker group
    echo "Trying with docker group..."
